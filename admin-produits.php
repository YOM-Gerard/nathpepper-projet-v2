                    ...
                    <?php else: ?>
                        <?php foreach ($products as $prod): ?>
                            <tr>
                                ...
                                <td>
                                    <?php 
                                    $current_stock = isset($prod['stock']) ? intval($prod['stock']) : 0;
                                    if ($current_stock === 0): 
                                    ?>
                                        <span style="background-color: rgba(239, 83, 80, 0.1); color: #ef5350; padding: 6px 12px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; border: 1px solid rgba(239, 83, 80, 0.3); display: inline-block;">
                                            ❌ RUPTURE
                                        </span>
                                    <?php elseif ($current_stock <= 5): ?>
                                        <span style="background-color: rgba(255, 167, 38, 0.1); color: #ffa726; padding: 6px 12px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; border: 1px solid rgba(255, 167, 38, 0.3); display: inline-block;">
                                            ⚠️ FAIBLE (<?php echo $current_stock; ?>)
                                        </span>
                                    <?php else: ?>
                                        <span style="background-color: rgba(102, 187, 106, 0.1); color: #66bb6a; padding: 6px 12px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; border: 1px solid rgba(102, 187, 106, 0.2); display: inline-block;">
                                            ✓ En stock (<?php echo $current_stock; ?>)
                                        </span>
                                    <?php endif; ?>
                                </td>
